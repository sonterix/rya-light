...
const AUDIENCE_CATEGORIES = ['Affluent Culture Seekers', 'Competitive Sports Fans', 'Emerging Tech Professionals', 'Wellness-Oriented Parents'];
...
const EDUCATIONS = ['High school graduate', 'Some college', 'Completed 4-year college', 'Post-graduate degree'];

const EMPLOYMENT_STATUSES = ['Work full-time', 'Work part-time', 'Retired'];
...
export function FilterControls({ filters, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <MultiSelect label="Gender" options={GENDERS} selected={genderValues} onChange={(values) => updateCategorical('gender', values)} />

      <MultiSelect label="Region" options={REGIONS} selected={regionValues} onChange={(values) => updateCategorical('region', values)} />

      <Button variant="ghost" size="sm" onClick={() => setShowAdvanced((prev) => !prev)} className="w-fit gap-1.5">
      ...
      {showAdvanced && (
        <div className="flex flex-col gap-5 border-t pt-4">
          <MultiSelect label="State" options={STATES} selected={stateValues} onChange={(values) => updateCategorical('state', values)} />
          ...
          <MultiSelect label="Education" options={EDUCATIONS} selected={educationValues} onChange={(values) => updateCategorical('education', values)} />
